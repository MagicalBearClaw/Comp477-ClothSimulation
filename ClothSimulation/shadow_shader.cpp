...
ShadowShader::ShadowShader(RenderContext* renderContext) : _renderContext(renderContext)
{
...
bool ShadowShader::Render(int indexCount, DirectX::XMFLOAT4X4 worldMatrix, DirectX::XMFLOAT4X4 viewMatrix, DirectX::XMFLOAT4X4 projectionMatrix, DirectX::XMFLOAT4X4 lightViewMatrix, DirectX::XMFLOAT4X4 lightProjectionMatrix, ID3D11ShaderResourceView* depthMapTexture, DirectX::XMFLOAT3 lightPosition)
{
	ID3D11DeviceContext* deviceContext = _renderContext->GetDeviceContext();

	bool result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, lightViewMatrix, lightProjectionMatrix, depthMapTexture, lightPosition);
	if (!result)
	{
		return false;
	}
...
bool ShadowShader::InitializeShader(ID3D11Device* device, HWND windowHandle, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	HRESULT result;
	...
	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	result = device->CreateBuffer(&lightBufferDesc2, NULL, &_lightBuffer);
	if (FAILED(result))
	{
		return false;
	}
...
void ShadowShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool ShadowShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, DirectX::XMFLOAT4X4 worldMatrix, DirectX::XMFLOAT4X4 viewMatrix, 
									   ...
									   ID3D11ShaderResourceView* depthMapTexture, DirectX::XMFLOAT3 lightPosition)
{
	...
	DirectX::XMMATRIX worldMatrixSim = DirectX::XMMatrixTranspose(DirectX::XMLoadFloat4x4(&worldMatrix));
	DirectX::XMMATRIX viewMatrixSim = DirectX::XMMatrixTranspose(DirectX::XMLoadFloat4x4(&viewMatrix));
	DirectX::XMMATRIX projectionMatrixSim = DirectX::XMMatrixTranspose(DirectX::XMLoadFloat4x4(&projectionMatrix));
	DirectX::XMMATRIX lightViewMatrixSim = DirectX::XMMatrixTranspose(DirectX::XMLoadFloat4x4(&lightProjectionMatrix));
	...
	DirectX::XMStoreFloat4x4(&dataPtr->world, worldMatrixSim);
	DirectX::XMStoreFloat4x4(&dataPtr->view, viewMatrixSim);
	DirectX::XMStoreFloat4x4(&dataPtr->projection, projectionMatrixSim);
	DirectX::XMStoreFloat4x4(&dataPtr->lightView, lightViewMatrixSim);
	...
	deviceContext->VSSetConstantBuffers(bufferNumber, 1, &_lightBuffer);
